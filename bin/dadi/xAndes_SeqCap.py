...
Description: Run dadi 5x on fs from sequence capture data.

Usage: python xAndes_SeqCap.py 
...
for i in range(5):
	...
	dir = ("/scratch/mgharvey/SysBio/dadi/SeqCap")
	os.chdir(dir)
	outfile = open("./optimized_output_SeqCap_{0}.txt".format(i+1), 'wb')

	data = dadi.Spectrum.from_file('SeqCap.fs')
